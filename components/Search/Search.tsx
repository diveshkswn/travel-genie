...
export function Search({placeholder = 'Search', handleSearch, className}: PropsTypes) {
  const {SEARCH_PROMPT} = constants;
  const [searchData, setSearchData] = useState('');
  return (
    <StyledSearch>
      <input placeholder={placeholder} className={className + " secondary-bg"} onChange={(e) => setSearchData(e.target.value)}/>
